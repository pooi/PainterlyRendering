...
void paintLayer(IplImage* canvas, IplImage* reference, int R, int mode) { // 가장 기초가 되는 함수 (색칠할 x,y들을 구하는 등)
	...
	for (int y = grid / 2; y <= size.height; y += grid) { // y값을 grid 값을 기준으로 반복함
		for (int x = grid / 2; x <= size.width; x += grid) { // x값을 grid 값을 기준으로 반복함
			...
			int x1, y1; // 연결 리스트 S에 저장될 x,y 좌표를 저장할 변수 (논문 상의 x1, y1)
...
void paint(IplImage* src, int * R, int R_length, int mode) {
	...
	for (int i = 0; i<R_length; i++) { // 브러쉬의 개수 만큼 반복
		...
		cvShowImage("canvas", canvas); // 각 각의 단계의 그림을 보여줌
		cvWaitKey(800);
...
int main() {
	...
	printf("Multimedia Programming Homework #3\n");
	printf("Painterly Rendering\n");
	printf("Made by Taewoo You\n");
	printf("=============================================\n\n");


	IplImage* src = NULL;

	while (1) { // 파일명을 입력받는 부분
		char fileName[200];
	...
	int mode = 0; // 0또는 1의 Mode를 받기 위해


	while (1) { // Mode를 입력받기 위해
		printf("Select Drawing Mode (0=circle, 1=stroke) : "); scanf("%d", &mode); // Mode를 입력받음
	...
	int R[] = { 32, 16, 8, 4, 2 }; // 브러쉬 사이즈를 저장 (논문 상의 Ri)

	int R_length = sizeof(R) / sizeof(int); // 브러쉬의 개수 파악

	paint(src, R, R_length, mode);
